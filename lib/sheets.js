...
const META_TEST_MODE = String(process.env.META_TEST_MODE || "").trim().toLowerCase() === "true";
...
async function sendMetaFeedbackEvent(lead, eventName) {
  ...
  if (META_TEST_MODE && META_TEST_EVENT_CODE) {
...
function rowToLead(headers, row, rowNumber, sheetName = SHEET_NAME) {
  ...
  // Meta uses COMPLETED for a submitted form; the finance workflow starts at CREATED.
  if (normalizeStatus(lead.lead_status) === "COMPLETED") lead.lead_status = "CREATED";
  return lead;
}
